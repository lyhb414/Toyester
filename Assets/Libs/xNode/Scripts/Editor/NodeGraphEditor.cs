...
namespace XNodeEditor
{
    ...
    public class NodeGraphEditor : XNodeEditor.Internal.NodeEditorBase<
        NodeGraphEditor, NodeGraphEditor.CustomNodeGraphEditorAttribute,
        XNode.NodeGraph>
    {
        ...
        {
            Vector2 pos =
                NodeEditorWindow.current.WindowToGridPosition(Event.current
                    .mousePosition);
            ...
            menu.AddSeparator("");
            menu.AddItem(new GUIContent("Maximize"), false,
                () => window.Maximize());
        ...
        public virtual Node CreateNode(Type type, Vector2 position)
        {
            ...
            if (string.IsNullOrEmpty(node.name))
            {
                ...
                if (typeName.EndsWith("Node"))
                    typeName =
                        typeName.Substring(0, typeName.LastIndexOf("Node"));
                node.name =
                    UnityEditor.ObjectNames.NicifyVariableName(typeName);
            }
        ...
        public class CustomNodeGraphEditorAttribute : Attribute,
            XNodeEditor.Internal.NodeEditorBase<NodeGraphEditor,
