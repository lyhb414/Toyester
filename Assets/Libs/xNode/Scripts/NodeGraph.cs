...
namespace XNode
{
    ...
    public abstract class NodeGraph : ScriptableObject
    {
        ...
        public void ChangeName()
        {
            ...
            var newPath =
                $"{Application.dataPath.Replace("Assets", "")}{Path.GetDirectoryName(path)}/{NewName}.asset";

            if (File.Exists(newPath))
            {
                Debug.LogError($"命名冲突，新路径已经有相应的资源 {newPath}");
                return;
            }

            File.Copy(absolutePath, newPath, false);
